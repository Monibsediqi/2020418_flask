...
class BaseModel(torch.nn.Module):
    ...
    def save_network(self, network, network_label, epoch_label, gpu_ids, opt):
        save_filename = '%s_net_%s.pth' % (epoch_label, network_label)
        save_path = os.path.join(self.save_dir, save_filename)
        state = {'opt': opt, 'weight': network.cpu().state_dict()}
    ...
    def load_network(self, network, network_label, epoch_label, save_dir=''):
        ...
        print("loading checkpoint from file %s" % save_path)
        if not os.path.isfile(save_path):
            ...
            try:
                # network.load_state_dict(torch.load(save_path))
                network.load_state_dict(state.get('weight', False))
            except:
                # pretrained_dict = torch.load(save_path)
                pretrained_dict = state.get('weight', False)
...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.cuda0 = opt.cuda0
        self.cuda1 = opt.cuda1
        self.data_parallel = opt.data_parallel
    ...
    def inference(self, label, inst, image=None):
        ...
        if self.use_features: # False
            ...
            print('xxxxx use_features false')
        ...
        if torch.__version__.startswith('0.4'):
            ...
            if self.data_parallel:
                ...
                fake_image = self.netG.forward(input_concat.cuda(self.cuda1))
        print('Inference (), fake_image shape: ', fake_image.shape)
        return fake_image

    # def forward(self, label, inst, image):
    #     return self.inference(label, inst, image)
...
class InferenceModel(Pix2PixHDModel):
    def forward(self, inp):
        label, inst = inp
        print('xxx InferenceModel', label.size(), inst.size())
